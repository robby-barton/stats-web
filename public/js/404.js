// Smart redirect logic for the 404 page (loaded via
// <script src="/js/404.js" defer>). Reads the serialized rankings JSON from
// #available-rankings and sends stale ranking/team URLs to a valid page.
(function () {
	...
	if (week.toLowerCase() !== 'final') {
		...
		if (!Number.isInteger(weekNum)) {
			window.location.replace(
				'/' + prefix + '/ranking/' + division + '/' + year + '/' + (postseason ? 'final' : weeks),
			);
			return;
		}
		if (weekNum > weeks) {
